...
def get_study_list():
    ...
    order_by = '{0} {1}'.format(['title','authors','journal','year','pmid'][int(request.args['order[0][column]'])],str(request.args['order[0][dir]']))
    ...
    result['data'] = [ ['<a href={0}>{1}</a>'.format(url_for('studies.show',val=d.pmid),d.title),
                          d.authors,
                          d.journal,
                          d.year,
