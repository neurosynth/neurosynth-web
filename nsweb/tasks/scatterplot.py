...
def scatter(x=None, y=None, gene=None, threshold_x=None, threshold_y=None,
    ...
    scale = 5
    if marginals:
        scatter_ax = plt.subplot2grid(
            (scale, scale), (1, 0), colspan=scale-1, rowspan=scale-1)
        top_ax = plt.subplot2grid((scale, scale), (0, 0), colspan=scale-1)
        right_ax = plt.subplot2grid(
            (scale, scale), (1, scale-1), rowspan=scale-1)
        top_ax.set_axis_off()
    ...
    for i, v in enumerate(np.unique(layers)):
        ...
        scatter_ax.plot(x_r, y_r, 'o', alpha=a, markersize=6,
        ...
        if marginals:
            ...
            sns.kdeplot(y_r, ax=right_ax, color=color,
                        vertical=True, shade=True)

    scatter_ax.legend(fontsize=lab_size, markerscale=1.5,
